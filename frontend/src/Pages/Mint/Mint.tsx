...
import {CHAINS, MINT_CHAINS, MORALIS_URL, TESTNET_MINT_NFT_CONTRACT} from "@/utils/constants.tsx";
...
const Mint = () => {
    ...
    const onSubmitCreate = async (formData: typeof form.values) => {
        ...
        const contractRead = new ethers.Contract(TESTNET_MINT_NFT_CONTRACT, MINT_NFT_ABI, customHttpProvider);
        ...
        <Box className={classes.mintLayoutWrapper} pt={'md'} w={'100%'}>
            <PageWrapper backIcon={<IconHome/>} title={'Mint Free NFT'} backLink={RoutePaths.BALANCE}>
                <ScrollArea.Autosize pt={'sm'} offsetScrollbars mah={height - 70}>
                    ...
                    {status === "connected" && (selectedChainType !== CHAINS.TESTNET) &&
                        <Alert my={'sm'} p={'xs'} variant="light" color="red" icon={<IconInfoCircle/>}>
                            Please connect to Areon Testnet
                        </Alert>
                    }
                    ...
                    <form style={{width: '100%'}} onSubmit={form.onSubmit(onSubmitCreate)}>
                        <Stack>
                            <TextInput
                                disabled={status !== 'connected' || (selectedChainType !== CHAINS.TESTNET)}
                                w={'100%'}
                                withAsterisk
                            ...
                            <Group justify="flex-end">
                                <Button
                                    disabled={form?.values?.attributes?.length >= 5 || status !== 'connected' || (selectedChainType !== CHAINS.TESTNET)}
